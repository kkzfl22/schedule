...
public class CheckTaskFinish implements FlowInf {
  ...
  public boolean invokeFlow(ContextContainer context) {
    ...
    logger.info("thread sqlmap.sqlmap.job flow check finish start ...");
    ...
    context.put(EtlThreadTaskEnum.PROC_RUN_FINISH_FLAG.name(), finish);
    logger.info("thread sqlmap.sqlmap.job flow check finish rsp {}", finish);
