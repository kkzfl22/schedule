...
@Service
@Slf4j
public class DcBatchInfoApplicationService {
     ...
     * @return true 操作成功,false 操作失败
     */
    public boolean update(DcBatchInfoDO param){
        boolean updateRsp = domainService.updateRunStatus(param);
        return updateRsp;
    }
    ...
    public DomainPage<List<DcBatchInfoDO>> queryPage(DomainPage<DcBatchInfoDO> pageReq){
        DomainPage<List<DcBatchInfoDO>> pageResult = domainService.queryPage(pageReq);
        return pageResult;
    }
    ...
    public DcBatchInfoDO detail(DcBatchInfoDO param){
        DcBatchInfoDO queryReturn = domainService.detail(param);
        return queryReturn;
    }
